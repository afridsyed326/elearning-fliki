...
export const userHasRequiredRole = async (userId: string, requiredRole: Role): Promise<boolean> => {
...
export const authorizeUser = async (userId: string, roles: Role[]) => {
    ...
    const roleCheckPromises = roles.map((role) => userHasRequiredRole(userId, role));
